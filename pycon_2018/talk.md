...
^ so I really truly do want some python packaging archeology with y'all
...
^ how people find it if they want it?
...
^ So at the "1998 international python conferece" (this is before it was pycon)
...
^ the idea was: why write a DSL when you have the full power of Python at your disposal
...
^ distutils also gave us a way to package up a source tree for sharing
...
^ I'm talking about linux package mangers like dpkg and rpm
...
^ so it's probably not appropriate to call it that anymore, given the hundreds of thousands of packages
...
^ An egg is simply a zip file, with some metadata, and binaries if necessary
...
^ this allows us to specify dependencies for an application
...
^ And the name comes from the old method of tying up packages with twine before sending them
...
^ one big difference here is the number of packages, from less than 3K to more than 130K
...
^ the other thing is that, by it's very nature, pypi predates most of the packages that exist on it

^ including all the web frameworks and testing frameworks you're familiar with

^ so it's fifteen years old, pretty much no tests, no modern web framework, what should we do?
...
^ of a core peice of infrastructure
...
^ project is called Warehouse, as in a place to put packages

^ officially became PyPI last month
...
# [fit] Python Packaging
# So Easy a Script Kiddie / Spammer Could Do It
...
^ we kinda assume that a) you have python

^ b) you really just want to do python (and of course, some C)
...
^ there's an additional problem with this as well
...
^ also, it's very difficult to use anything else, they are the standard interface for just about everything
...
^ to summarize: packaging isn't bad! but there are always problems to be solved

^ we've made gradual changes over a long period of time

^ and each change is a response to an evolving need

^ comparatively, it used to be really bad, and we might have for forgotten, or just never known, how bad it could be
